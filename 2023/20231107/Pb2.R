###----------------------------------------------------------###
### Problem 2: Capturing the essence of gemstones (20231107) ###
###----------------------------------------------------------###
...
# It makes sense to use standardized variables otherwise the PCs would basically be Area and ConvArea
...
# If we wanted to perform dimensionality reduction, we could keep 2/3 PCs
...
# The first PC represents an average of the measures regarding how big the gemstone is.
# The second PC contains the information about eccentricity and roundness prevalently.

# High PC1: big gemstone overall
# Low PC1: small gemstone overall
# High PC2: high eccentricity, small roundness
# Low PC2: small eccentricity, high roundness

scores.data <- pc.data$scores

col.lab <- ifelse(data.label[] %in% c("sapphire", "emerald"), 
                  ifelse(data.label[] %in% c("sapphire"), 
                         'blue', 'green'), 
                  'red')
...
legend('topright', levels(factor(data.label[])), fill = c('green', 'red', 'blue'), bty = 'n')

# The gemstones are pretty well clustered along the first PC's direction:
# sapphires are relatively small, emeralds are really big, rubys are halfway round


# c) ----------------------------------------------------------------------

scores.new <- scores.data[which(data.label == "ruby"), 1:2]
...
mvn(scores.new)$multivariateNormality$'p value'

# At 1% significance level, we can confirm the data are normal